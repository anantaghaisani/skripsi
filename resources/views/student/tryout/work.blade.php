...
@section('title', $tryout->title . ' - Hakuna Matata Course')
        ...
        <div class="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-xl p-6 text-white shadow-lg mb-6">
            <div class="flex items-center justify-between">
                <div class="flex-1">
                    <div class="flex items-center space-x-3 mb-4">
                        ...
                        <div>
                            ...
                            <p class="text-xl font-bold">{{ $tryout->code }}</p>
                        </div>
                    </div>
                ...
                <div class="text-center bg-white/20 rounded-xl px-8 py-4 ml-6">
                    ...
                    <div id="timer" class="text-4xl font-bold" data-remaining="{{ $remainingMinutes }}">
                        <span id="timer-display">{{ floor($remainingMinutes) }}:00</span>
                    </div>
                            ...
                            <div class="relative">
                                <button 
                                    ...
                                    class="nav-button w-full aspect-square rounded-lg font-semibold text-sm transition
                                           {{ isset($userAnswers[$question->id]) ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-600 hover:bg-gray-300' }}">
                                    {{ $index + 1 }}
                                </button>
                                <!-- Flag Icon -->
                                <button 
                                    onclick="toggleFlag({{ $question->id }}); event.stopPropagation();"
                                    id="flag-{{ $question->id }}"
            ...
            <div class="lg:col-span-3">
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
                    ...
                    @foreach($tryout->questions as $index => $question)
                        <div 
                            id="question-{{ $index + 1 }}" 
                            class="question-container {{ $index === 0 ? '' : 'hidden' }}">
                            ...
                            <div class="mb-6">
                                <div class="flex items-center mb-4">
                                    ...
                                    <div>
                                        <p class="text-sm text-gray-500">Soal {{ $index + 1 }} dari {{ $tryout->questions->count() }}</p>
                                        <p class="text-xs text-gray-400">Poin: {{ $question->points }}</p>
                                    </div>
                                </div>
                                ...
                                @foreach($question->answers as $answer)
                                    <label 
                                        class="block p-4 border-2 rounded-lg cursor-pointer transition
                                               {{ isset($userAnswers[$question->id]) && $userAnswers[$question->id]->answer_id == $answer->id ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50' }}">
                                        <div class="flex items-start">
                                            <input 
                                                ...
                                                {{ isset($userAnswers[$question->id]) && $userAnswers[$question->id]->answer_id == $answer->id ? 'checked' : '' }}
                                                class="mt-1 w-5 h-5 text-blue-600">
                                            <div class="ml-4 flex-1">
                                                <div class="flex items-start">
                                                    <span class="font-semibold text-gray-700 mr-3">{{ $answer->option }}.</span>
                                                    <div class="flex-1">
                            ...
                            <div class="flex items-center justify-between mt-8 pt-6 border-t">
                                ...
                                @if($index < $tryout->questions->count() - 1)
                                    <button 
...
<!-- Submit Form (Hidden) -->
<form id="submit-form" action="{{ route('tryout.submit', $tryout->id) }}" method="POST" class="hidden">
    @csrf
</form>
...
const totalQuestions = {{ $tryout->questions->count() }};
const tryoutId = {{ $tryout->id }};
let remainingSeconds = {{ floor($remainingMinutes * 60) }};
