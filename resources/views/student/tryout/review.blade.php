...
<div class="p-8">
    <div class="max-w-5xl mx-auto">
                ...
                @foreach($tryout->questions as $index => $question)
                    ...
                    <a href="#question-{{ $question->id }}" 
                       class="w-10 h-10 rounded-lg font-semibold text-sm flex items-center justify-center transition
                              {{ $isAnswered ? ($isCorrect ? 'bg-green-500 text-white' : 'bg-red-500 text-white') : 'bg-gray-300 text-gray-700' }}
                              hover:opacity-80">
                        {{ $index + 1 }}
                    </a>
                @endforeach
            ...
            @foreach($tryout->questions as $index => $question)
                ...
                <div id="question-{{ $question->id }}" 
                     class="bg-white rounded-xl shadow-sm border-2 p-8
                            {{ $isAnswered ? ($isCorrect ? 'border-green-500' : 'border-red-500') : 'border-gray-300' }}">
                    
                    <!-- Question Header -->
                    <div class="flex items-start justify-between mb-6">
                        <div class="flex items-center">
                            <div class="w-12 h-12 rounded-lg flex items-center justify-center mr-4
                                        {{ $isAnswered ? ($isCorrect ? 'bg-green-100' : 'bg-red-100') : 'bg-gray-100' }}">
                                @if($isAnswered)
                                    @if($isCorrect)
                            ...
                            <div>
                                <p class="text-sm font-semibold
                                          {{ $isAnswered ? ($isCorrect ? 'text-green-600' : 'text-red-600') : 'text-gray-500' }}">
                                    Soal {{ $index + 1 }}
                                </p>
                                <p class="text-xs text-gray-500">Poin: {{ $question->points }}</p>
                            </div>
                        </div>
                    ...
                    <div class="mb-6">
                        <div class="prose max-w-none">
                            <p class="text-lg text-gray-900 leading-relaxed mb-4">{{ $question->question_text }}</p>
                    ...
                    <div class="space-y-3 mb-6">
                        @foreach($question->answers as $answer)
                            ...
                            <div class="p-4 border-2 rounded-lg
                                        {{ $isCorrectAnswer ? 'border-green-500 bg-green-50' : ($isUserAnswer ? 'border-red-500 bg-red-50' : 'border-gray-200') }}">
                                <div class="flex items-start">
                                    <!-- Option Badge -->
                                    <div class="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center mr-3
                                                {{ $isCorrectAnswer ? 'bg-green-500 text-white' : ($isUserAnswer ? 'bg-red-500 text-white' : 'bg-gray-200 text-gray-700') }}">
                                        <span class="font-bold text-sm">{{ $answer->option }}</span>
                                    </div>
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <p class="text-gray-900 {{ $isCorrectAnswer || $isUserAnswer ? 'font-semibold' : '' }}">
                                                    {{ $answer->answer_text }}
                                                </p>
        ...
        <div class="mt-8 flex items-center justify-between bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <a href="{{ route('tryout.result', $tryout->id) }}" 
