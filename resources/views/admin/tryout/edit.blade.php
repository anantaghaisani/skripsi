    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="text-sm">
                ...
                <span class="ml-2 font-mono font-bold text-red-600">{{ $tryout->token }}</span>
            </div>
        </div>

        <form action="{{ route('admin.tryout.update', $tryout->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea id="description" 
                          name="description" 
                          rows="3"
                          class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">{{ old('description', $tryout->description) }}</textarea>
                @error('description')
                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t">
                ...
                <button type="submit" 
                        class="px-6 py-3 bg-[#DC2626] hover:bg-red-700 text-white font-semibold rounded-lg transition shadow-sm">
                    Simpan Perubahan
                </button>
            </div>
        </form>
