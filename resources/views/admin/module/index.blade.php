...
@section('page-title', 'Kelola Modul')
...
<div class="p-8">
    ...
    <div class="mb-6">
        <div class="flex bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <!-- Tab: Semua -->
            <a href="{{ route('admin.module.index', request()->except('is_active')) }}"
            ...
            <!-- Tab: Aktif -->
            <a href="{{ route('admin.module.index', array_merge(request()->except('is_active'), ['is_active' => '1'])) }}"
               class="flex-1 py-4 px-6 text-center text-sm transition
               {{ request('is_active') === '1'
                    ? 'font-semibold text-[#DC2626] border-b-4 border-[#FFBF00]'
                    : 'font-medium text-gray-400 hover:text-gray-600' }}">
            ...
            <!-- Tab: Non Aktif -->
            <a href="{{ route('admin.module.index', array_merge(request()->except('is_active'), ['is_active' => '0'])) }}"
               class="flex-1 py-4 px-6 text-center text-sm transition
               {{ request('is_active') === '0'
                    ? 'font-semibold text-[#DC2626] border-b-4 border-[#FFBF00]'
                    : 'font-medium text-gray-400 hover:text-gray-600' }}">
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 w-8 h-8 bg-gray-200 rounded-full flex items-center justify-center">
                                            <span class="text-xs font-semibold text-gray-600">{{ substr($module->creator->name, 0, 2) }}</span>
                                        </div>
                                        <div class="ml-3">
                                            <div class="text-sm font-medium text-gray-900">{{ $module->creator->name }}</div>
                                            <div class="text-xs text-gray-500">{{ ucfirst($module->creator->role) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <!-- Toggle Status -->
                                        <form action="{{ route('admin.module.toggle-status', $module->id) }}" method="POST" class="relative group">
                                            @csrf
                                            <button type="submit" 
                                        ...
                                        <!-- Hapus -->
                                        <form action="{{ route('admin.module.destroy', $module->id) }}" method="POST" class="relative group">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" 
