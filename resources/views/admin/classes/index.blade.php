    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="mb-6">
            <div class="flex bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <!-- Tab: Semua -->
                <a href="{{ route('admin.classes.index', request()->except('grade_level')) }}"
                   class="flex-1 py-4 px-6 text-center text-sm transition
                   {{ !request()->has('grade_level')
                ...
                <!-- Tab: SD -->
                <a href="{{ route('admin.classes.index', array_merge(request()->except('grade_level'), ['grade_level' => 'SD'])) }}"
                   class="flex-1 py-4 px-6 text-center text-sm transition
                   {{ request('grade_level') === 'SD'
                        ? 'font-semibold text-[#DC2626] border-b-4 border-[#FFBF00]'
                        : 'font-medium text-gray-400 hover:text-gray-600' }}">
                    SD
                </a>
                ...
                <!-- Tab: SMP -->
                <a href="{{ route('admin.classes.index', array_merge(request()->except('grade_level'), ['grade_level' => 'SMP'])) }}"
                   class="flex-1 py-4 px-6 text-center text-sm transition
                   {{ request('grade_level') === 'SMP'
                        ? 'font-semibold text-[#DC2626] border-b-4 border-[#FFBF00]'
                        : 'font-medium text-gray-400 hover:text-gray-600' }}">
                    SMP
                </a>
                ...
                <!-- Tab: SMA -->
                <a href="{{ route('admin.classes.index', array_merge(request()->except('grade_level'), ['grade_level' => 'SMA'])) }}"
                   class="flex-1 py-4 px-6 text-center text-sm transition
                   {{ request('grade_level') === 'SMA'
                        ? 'font-semibold text-[#DC2626] border-b-4 border-[#FFBF00]'
                        : 'font-medium text-gray-400 hover:text-gray-600' }}">
                    SMA
                </a>
                ...
                @foreach($classes as $class)
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-lg transition">
                        <div class="p-6">
                            ...
                            <h3 class="text-xl font-bold text-gray-900 mb-2">{{ $class->full_name }}</h3>
                            ...
                            <div class="flex items-center justify-center space-x-2">
                                ...
                                @if($class->students_count === 0)
                                    <form action="{{ route('admin.classes.destroy', $class->id) }}" method="POST" class="relative group">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
