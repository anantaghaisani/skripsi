...
<div class="p-8">
    ...
    <div class="mb-6">
        <div class="flex bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <!-- Tab: Student -->
            <a href="{{ route('admin.users.index', array_merge(request()->except('role'), ['role' => 'student'])) }}"
               class="flex-1 py-4 px-6 text-center text-sm transition
            ...
            <!-- Tab: Tentor -->
            <a href="{{ route('admin.users.index', array_merge(request()->except('role'), ['role' => 'tentor'])) }}"
               class="flex-1 py-4 px-6 text-center text-sm transition
               {{ request('role') === 'tentor'
                    ? 'font-semibold text-[#DC2626] border-b-4 border-[#FFBF00]'
                    : 'font-medium text-gray-400 hover:text-gray-600' }}">
                            ...
                            <tr class="hover:bg-gray-50 transition">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        ...
                                        @else
                                            <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&size=40&background=random" 
                                                 alt="{{ $user->name }}" 
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center space-x-2">
                                        ...
                                        <!-- Toggle Status -->
                                        @if($user->id !== auth()->id())
                                            <form action="{{ route('admin.users.toggle-status', $user) }}" method="POST" class="relative group">
                                                @csrf
                                                <button type="submit" 
                                                        class="inline-flex items-center justify-center w-9 h-9 {{ $user->is_active ? 'bg-gray-500 hover:bg-gray-600' : 'bg-green-500 hover:bg-green-600' }} text-white rounded-lg transition">
                                                    @if($user->is_active)
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <!-- Hapus -->
                                        @if($user->id !== auth()->id())
                                            <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST" class="relative group">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
